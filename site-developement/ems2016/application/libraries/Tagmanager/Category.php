...
class TagManager_Category extends TagManager
{
    ...
    if ( ! isset(self::$categories[$lsk]))
    {
      ...
      else
      {
        if($element_name == 'page')
          ...
          $id_page = NULL;
        $order_by = (string)$tag->getAttribute('order_by');
  ...
  public static function get_all_categories(FTL_Binding $tag) {
    // Categories model
    self::$ci->load->model('category_model');

    $order_by = (string)$tag->getAttribute('order_by');
    $categories = self::$ci->category_model->get_all_categories($order_by);
  ...
  public static function tag_categories(FTL_Binding $tag)
  {
    ...
    (bool)$get_all = $tag->getAttribute('get_all');
    if (!is_null($get_all) and $get_all) 
      $categories = self::get_all_categories($tag);
    else $categories = self::get_categories($tag);
